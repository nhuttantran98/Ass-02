...
void setBookData(vector<Book> a) {
	int size = a.size();
	fstream datafile("listbook.dat", ios::out | ios::binary);
	for (int i = 0; i < size; i++) {
		datafile.write(reinterpret_cast<char*>(&a[i]), sizeof(Book));
	}
	datafile.close();
}

void setBorrowedSheetData(vector<BorrowedSheet> a) {
	int size = a.size();
	fstream datafile("listBorrowedSheet.dat", ios::out | ios::binary);
	for (int i = 0; i < size; i++) {
		datafile.write(reinterpret_cast<char*>(&a[i]), sizeof(BorrowedSheet));
	}
	datafile.close();
}
